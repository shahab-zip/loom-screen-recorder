...
export function VideoPlayer({ video, onClose, onRename, onDelete, toggleWatchLater, isInWatchLater }: VideoPlayerProps) {
  ...
  // ── Right panel content ───────────────────────────────
  // ── Summary / Chapters / Tags helpers ────────────────
  const handleAddChapter = () => {
    setChapters(prev => [...prev, { id: Date.now().toString(), time: Math.floor(currentTime), title: 'New chapter' }]);
  };
      ...
      case 'edit':
        return (
          <div className="p-4 space-y-5">
            {/* AI banner — slim, single line */}
            <button className="w-full flex items-center justify-between gap-2 px-3 py-2.5 bg-gradient-to-r from-violet-50 to-blue-50 border border-violet-100 rounded-lg hover:border-violet-200 transition-colors text-left">
              <div className="flex items-center gap-2 min-w-0">
                <Sparkles className="w-3.5 h-3.5 text-violet-600 flex-shrink-0" />
                <span className="text-xs font-semibold text-gray-800 truncate">Loom Business + AI</span>
              </div>
              <span className="text-xs text-violet-600 font-semibold flex-shrink-0">Upgrade</span>
            </button>

            {/* Edit & Enhance — compact rows */}
            <div>
              <h3 className="text-[11px] font-semibold text-gray-400 uppercase tracking-wider mb-2 px-1">Edit and enhance</h3>
              <div className="space-y-0.5">
                {[
                  { icon: Scissors, label: 'Edit and trim video, add clips, links, and text overlays', pro: true },
                  { icon: Mic, label: 'Remove silences & filler words', pro: true },
                  { icon: Sparkles, label: 'Auto-title, chapters, and summary', pro: true },
                ].map((tool) => {
                  ...
                  return (
                    ...
                    >
                      <Icon className="w-4 h-4 text-gray-400 group-hover:text-gray-600 flex-shrink-0 transition-colors" />
                      <span className="flex-1 text-xs text-gray-700 leading-snug">{tool.label}</span>
                      {tool.pro && <Lock className="w-3 h-3 text-gray-300 flex-shrink-0" />}
                    </button>
                  );
                })}
              </div>
    ...
    <div className="flex flex-col h-full bg-white animate-fade-in">
      ...
      <header className="flex items-center justify-between px-5 py-3 border-b border-gray-100 bg-white/95 backdrop-blur-sm flex-shrink-0 z-10">
        ...
        <div className="flex items-center gap-3 min-w-0 flex-1">
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="flex items-center gap-1.5 text-xs text-gray-400 mt-0.5">
              <div className="w-4 h-4 rounded-full bg-gradient-to-br from-red-500 to-red-600 flex items-center justify-center text-white text-[8px] font-bold flex-shrink-0">
                ME
              </div>
              <span>You</span>
              <span>·</span>
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          <div className="hidden sm:flex items-center gap-1 px-2.5 py-1 rounded-full bg-gray-50 text-xs font-medium text-gray-500">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        {/* ── Left column: video + reactions + summary/chapters/tags ── */}
        <div className="flex flex-col flex-1 min-w-0 overflow-y-auto">

          {/* Video player */}
          <div
            ref={containerRef}
            className="relative aspect-video w-full bg-gray-950 group cursor-pointer overflow-hidden flex-shrink-0"
          ...
          <div className="bg-white border-t border-gray-100 px-6 py-3 flex items-center justify-between flex-shrink-0">
            ...
            <button
              onClick={() => { setActiveTab('activity'); setSidebarOpen(true); }}
          ...
          {/* ── Summary / Chapters / Tags ───────────── */}
          <div className="px-6 py-5 space-y-6 max-w-3xl">
            {/* Summary */}
            <section>
              <h2 className="text-base font-bold text-gray-900 mb-2">Summary</h2>
              <textarea
                value={summary}
                onChange={(e) => setSummary(e.target.value)}
                placeholder="Add a summary…"
                rows={summary ? Math.min(6, Math.max(2, summary.split('\n').length)) : 1}
                className="w-full text-sm text-gray-700 placeholder-gray-300 bg-transparent border-none focus:outline-none resize-none"
              />
            </section>

            {/* Chapters */}
            <section>
              <h2 className="text-base font-bold text-gray-900 mb-2">Chapters</h2>
              <div className="space-y-1.5">
                {chapters.map((c) => (
                  <div key={c.id} className="flex items-center gap-2 group">
                    <button
                      onClick={() => {
                        if (videoRef.current) videoRef.current.currentTime = c.time;
                        setCurrentTime(c.time);
                      }}
                      className="text-xs font-semibold text-red-600 bg-red-50 px-2 py-0.5 rounded tabular-nums hover:bg-red-100 transition-colors flex-shrink-0"
                    >
                      {formatTime(c.time)}
                    </button>
                    <input
                      type="text"
                      value={c.title}
                      onChange={(e) => handleUpdateChapter(c.id, e.target.value)}
                      className="flex-1 text-sm text-gray-700 bg-transparent border-none focus:outline-none focus:bg-gray-50 px-2 py-1 rounded"
                    />
                    <button
                      onClick={() => handleRemoveChapter(c.id)}
                      className="opacity-0 group-hover:opacity-100 text-gray-300 hover:text-red-600 transition-all flex-shrink-0 p-1"
                ...
                <button
                  ...
                  className="text-sm text-gray-400 hover:text-red-600 transition-colors py-1"
                >
                  + Add chapters
                </button>
              </div>
            ...
            <section>
              <div className="flex flex-wrap items-center gap-2">
                {tags.map((t) => (
                  <span key={t} className="inline-flex items-center gap-1 px-2.5 py-1 bg-gray-100 text-xs font-medium text-gray-700 rounded-full">
                    #{t}
                    <button
                ...
                <div className="inline-flex items-center gap-1 px-2.5 py-1 border border-dashed border-gray-200 hover:border-gray-300 rounded-full transition-colors">
                  ...
                  <input
                    ...
                    onBlur={handleAddTag}
                    placeholder="add tag"
                    className="text-xs bg-transparent border-none focus:outline-none placeholder-gray-300 w-20"
                  />
                </div>
              </div>
            </section>
        ...
        {sidebarOpen ? (
            ...
            <div className="flex items-center border-b border-gray-100 flex-shrink-0 px-2">
              {([
                ...
                <button
                  ...
                  className={`px-2.5 py-3 text-sm font-medium transition-colors border-b-2 -mb-px ${
                    activeTab === id
              ...
              <button
                ...
                className="ml-auto p-1.5 text-gray-300 hover:text-gray-600 hover:bg-gray-50 rounded transition-colors"
          ...
          <button
            ...
            title="Show panel"
            className="hidden lg:flex flex-col items-center justify-start py-3 px-2 border-l border-gray-100 bg-white text-gray-400 hover:text-gray-700 hover:bg-gray-50 transition-colors"
